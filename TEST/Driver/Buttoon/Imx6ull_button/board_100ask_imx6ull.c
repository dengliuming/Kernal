...
int Read (int whitch,char* buff,size_t size) {
    printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
    if (whitch == 0) {
        buff = (char*)(gpio5->psr & (1<<1)) ? 1 : 0;
        size  =sizeof(buff);
    } 
    else if (whitch == 1) {
         buff =  (char*)(gpio4->psr & (1<<14)) ? 1 : 0;
        size  =sizeof(buff);
    }
    else {
        size = -1;
...
static struct Button_Imx6ull button_imx6ull = {
    .count = 2,
    .init = Init,
    .read = Read,
    .write = Write
...
struct Button_Imx6ull* GetButtonImx6ull (void) {
    return &button_imx6ull;
}

int ButtonImx6ull_Init (void) {
    RegestButtonImx6ll(&button_imx6ull);
    return 0;
}

void ButtonImx6ull_Exit (void) {
    UnregestButtonImx6ll(&button_imx6ull);
}

module_init (ButtonImx6ull_Init);
module_exit (ButtonImx6ull_Exit);
MODULE_LICENSE("GPL");
